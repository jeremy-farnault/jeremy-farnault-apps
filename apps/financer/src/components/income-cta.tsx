...
import { addIncomeEntry, createIncomeSource } from "@/lib/actions";
import { CURRENCIES } from "@/lib/constants";
import type { IncomeSourceRow } from "@/lib/queries";
import { ActionModal, Select, SelectItem, TextInput } from "@jf/ui";
import { HandCoinsIcon, MinusIcon, PlusCircleIcon, PlusIcon } from "@phosphor-icons/react";
import { useState, useTransition } from "react";
import { toast } from "sonner";

interface IncomeCtaProps {
  viewedMonth: string;
  homeCurrency: string;
  sources: IncomeSourceRow[];
}

type IncomeLogRow = { sourceId: string; currency: string; value: string };
...
export function IncomeCta({ viewedMonth, homeCurrency, sources }: IncomeCtaProps) {
  ...
  const [rows, setRows] = useState<IncomeLogRow[]>([
  ...
  function handleAddSourceSubmit() {
    ...
    startAddSourceTransition(async () => {
      try {
        await createIncomeSource({ name, currency: sourceCurrency });
  ...
  function updateRow(idx: number, field: keyof IncomeLogRow, val: string) {
