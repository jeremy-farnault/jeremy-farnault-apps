...
function parseArgs(): { color: string; out: string; icon: string } {
  ...
  if (!color || !out) {
    console.error(
      "Usage: tsx scripts/generate-app-icon.ts --color <hex> --out <dir> [--icon barbell|coins|ranking|map-pin|pill|infinity|robot|aperture|kanban|atom]"
    );
    process.exit(1);
  }
